...
export class UsersController {
	...
	@Get('/user')
	@Summary(
		'This endpoint loads a user and his roles from Almond Backend system'
	)
	@Description('Return a user with a given email')
	@Returns(200, AlmondUser)
	async getUserByEmail(
		@Res() response: PlatformResponse,
		@Format('email') @Required() @QueryParams('email') email: string
	): Promise<PlatformResponse<AlmondUser>> {
		try {
			...
			return response.body(
				ResponseWrapper.FailResponse(`An error occurred ${e.message}`, data)
			);
		}
	}
	...
	async getAllUsers(@Res() response: PlatformResponse) {
		try {
			...
			return response.body(
				ResponseWrapper.SuccessResponse('Fetched Successfully', users)
			);
		} catch (e: any) {
			const data = null;
			return response.body(
				ResponseWrapper.FailResponse(`Failed to load users ${e.message}`, data)
			);
		}
	}
}
