...
export class RolesController {
	...
	async createOrUpdateRole(
		...
		@BodyParams() roleModel: RolesModel
	) {
		try {
			const role = await this.roleService
				.createOrUpdateRole(roleModel)
				.then((data) => JSON.stringify(data));
			return response.body(
				ResponseWrapper.SuccessResponse('Operation was Successful', role)
			);
		} catch (e) {
			return response.body(
	...
	async getRoles(@Res() response: PlatformResponse) {
		try {
			...
			return response.body(
				ResponseWrapper.SuccessResponse('Records fetched successfully', roles)
			);
		} catch (e: any) {
			return response.body(
	...
	async findRoleById(
		...
		try {
			...
			return response.body(
				ResponseWrapper.SuccessResponse('Record fetched successfully', role)
			);
		} catch (e: any) {
			return response.body(
				ResponseWrapper.FailResponse(
	...
	async deleteRole(
		...
		try {
			...
			return response.body(
				ResponseWrapper.SuccessResponse('Record deleted successfully', role)
			);
		} catch (e: any) {
			return response.body(
				ResponseWrapper.FailResponse(
