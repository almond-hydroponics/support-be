...
export class ProfilesController {
	...
	async createOrUpdateProfile(
		...
		@BodyParams() userModel: UserModel
	) {
		try {
			const profile = await this.userService
				.createProfile(userModel)
				.then((data) => JSON.stringify(data));
			return response.body(
				ResponseWrapper.SuccessResponse('Operation was Successful', profile)
			);
		} catch (e) {
			return response.body(
	...
	async getProfiles(@Res() response: PlatformResponse) {
		try {
			...
			return response.body(
				ResponseWrapper.SuccessResponse('Failed to fetch', e.message)
			);
		}
	}
	...
	async findProfileById(
		...
		try {
			...
			return response.body(
				ResponseWrapper.SuccessResponse('Record fetched successfully', profiles)
			);
		} catch (e: any) {
			return response.body(
				ResponseWrapper.SuccessResponse(
	...
	async deleteProfile(
		...
		try {
			...
			return response.body(
				ResponseWrapper.SuccessResponse('Record deleted successfully', profiles)
			);
		} catch (e: any) {
			return response.body(
				ResponseWrapper.SuccessResponse(
